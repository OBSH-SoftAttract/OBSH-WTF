...
public class HotelStaffDataMysqlHelper implements HotelStaffDataHelper {
	...
	public Map<Integer, UserPo> getHotelStaffData() {
	    ...
	    sql = "select *from HotelStaff";//SQL语句  
	...
	public void addHotelStaffData(UserPo userPo) {
		...
		sql = "insert into HotelStaff value("+userPo.getId()+", '"+userPo.getUsername()+"' ,"+userPo.getPassword()+","+userPo.getPhone()+")";
		db1 = new JDBCHelper(sql);//创建DBHelper对象  
		try {
